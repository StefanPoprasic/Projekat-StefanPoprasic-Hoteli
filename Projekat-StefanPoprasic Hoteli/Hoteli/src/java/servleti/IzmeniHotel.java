...
public class IzmeniHotel extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet IzmeniHotel</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet IzmeniHotel at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(idHotel!=null && idHotel.length()>0 && ime!=null && ime.length()>0
            ...
            try{
                ...
                st.executeUpdate("update hotel set idHotel='"+idHotel+"',ime='"+ime+"',idGrad='"+idGrad+"'where idHotel='"+idHotel+"'");
                st.close();
                con.close();
