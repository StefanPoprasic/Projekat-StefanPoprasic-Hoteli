...
public class DodajSobu extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet DodajSobu</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet DodajSobu at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(idHotel!=null && idHotel.length()>0 && tipSobe!=null && tipSobe.length()>0
                && idSoba!=null && idSoba.length()>0){
            try{
                Class.forName("com.mysql.jdbc.Driver");
                Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/hoteli2", "root", "");
                Statement st=con.createStatement();
                
                 st.executeUpdate("insert into soba(idSoba,tipSobe,idHotel,cena) values('"+idSoba
                +"','"+tipSobe+"','"+idHotel+"','"+cena+"');");
                st.close();
                con.close();
                request.setAttribute("msq","Dodavanje sobe je uspesno");
                request.getRequestDispatcher("Sobe").forward(request, response);
